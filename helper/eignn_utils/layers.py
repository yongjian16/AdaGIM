...
class EIGNN_k_steps(nn.Module):
    """
    Runs the EIGNN update:
        Z_{t+1} = gamma * (S^T Z_t) @ g(F)^T + X
    for exactly k steps, starting from Z_0 = 0 (or provided init).

    Notes:
      - S is expected to be a torch sparse adjacency (COO) or dense tensor.
      - This class does NOT use implicit differentiation; it backprops through the k unrolled steps.
    """
    def __init__(self, adj, m: int, k_steps: int = 10, gamma: float = 0.95):
        super().__init__()
        self.F = nn.Parameter(torch.FloatTensor(m, m), requires_grad=True)
        self.gamma = float(gamma)
        self.k_steps = int(k_steps)

        # store adjacency (sparse or dense)
        self.S = adj
        self.reset_parameters()

    def reset_parameters(self, gain: float = 0.99):
        nn.init.xavier_uniform_(self.F, gain=gain)


    def forward(self, X: torch.Tensor, z_init: Optional[torch.Tensor] = None) -> torch.Tensor:
        """
        X: [m,N]
        z_init: optional [m,N], defaults to zeros
        returns: Z_k [m,N]
        """
        assert X.dim() == 2, "X must be [m,N]"
        m, N = X.shape
        if z_init is None:
        ...
        for _ in range(self.k_steps):
            Z = self.gamma * self.F @ torch.sparse.mm(S_t, Z.t()).t() + X

        return Z


class EIGNN_w_iterative_solvers(nn.Module):
    def __init__(self, adj, sp_adj, m, threshold, max_iter, gamma, spectral_radius_mode=False, compute_jac_loss=False, g_type="psd"):
        ...
        self.g_type = g_type
        self.s = nn.Parameter(torch.zeros(m), requires_grad=True)
        # self.Lambda_S, self.Q_S = sp.linalg.eigsh(sp_adj, k=num_eigenvec, return_eigenvectors=True)
        # sp_adjs = (sp_adjs+)
        # ipdb.set_trace()
        self.max_iter = max_iter
    ...
    def reset_parameters(self, gain=0.99):
        torch.nn.init.xavier_uniform_(self.F, gain=gain)
        torch.nn.init.normal_(self.s, mean=0.0, std=0.01)
    ...
    def _inner_func(self, Z, X):
        ...
        if self.g_type == "psd":
            Z_new = self.gamma * g(self.F) @ torch.sparse.mm(S_t, Z.t()).t() + X
        elif self.g_type == "nonpsd2":
            Z_new = self.gamma * g_indef_F(self.F) @ torch.sparse.mm(S_t, Z.t()).t() + X
        elif self.g_type == "nonpsd1":
            Z_new = self.gamma * g_indef(self.F, self.s) @ torch.sparse.mm(S_t, Z.t()).t() + X
        return Z_new
...
class EIGNN_scale_w_iter(nn.Module):
    def __init__(self, adj, sp_adj, m, k, threshold, max_iter, gamma, layer_norm=False, spectral_radius_mode=False, psd_gF=False):
        ...
        self.gamma = nn.Parameter(torch.tensor(gamma, dtype=torch.float), requires_grad=False)
        self.k = k
        self.s = nn.Parameter(torch.zeros(m))
        self.psd_gF = psd_gF
...
def g_indef(F, s, eps=1e-12):
    d = torch.tanh(s)                         # [m]
    D = torch.diag(d)                         # [m,m]
    FDF = F.t() @ D @ F                         # symmetric, indefinite possible
    return (1 / (torch.norm(FDF, p="fro") + eps)) * FDF

# def g_indef_F(F, eps=1e-12):
#     F_norm = torch.norm(F, p="fro")
#     return (1 / (F_norm + epsilon_F)) * F
