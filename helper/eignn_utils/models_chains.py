...
from .layers import ImplicitGraph, IDM_SGC, IDM_SGC_topk, EIGNN_w_iterative_solvers, EIGNN_new_iter, EIGNN_exact_u, EIGNN_forward_iter, EIGNN_w_iter_adap_gamma, EIGNN_scale_w_iter
...
class ParityEIGNN(nn.Module):
    def __init__(self, adj, sp_adj, in_dim, hidden, out_dim=2, threshold=1e-4, max_iter=300, gamma=0.95):
        super(ParityEIGNN, self).__init__()
        # self.B = nn.Parameter(torch.FloatTensor(m_y, m), requires_grad=True)
        self.EIGNN = EIGNN_w_iterative_solvers(adj, sp_adj, hidden, threshold, max_iter, gamma)
    ...
    def forward(self, X):
        ...
        # print how similar between rows of output
        sim = output.detach() @ output.detach().T
        sim = sim.mean().item()
        print(f'similarity between rows: {sim}')
        logits = self.B(output)
        return logits
...
class EIGNN_w_iterative(nn.Module):
    def __init__(self, adj, sp_adj, m, m_y, threshold, max_iter, gamma, chain_len, adaptive_gamma=False):
        ...
        self.B = nn.Linear(m, m_y)
        self.embeddings = nn.Parameter(torch.rand(m, m_y), requires_grad=True)
        # self.F = nn.Parameter(torch.FloatTensor(m, m), requires_grad=True)
        # S_dense = S.to_dense()
        # self.S = S
        # self.gamma = nn.Parameter(torch.tensor(gamma, dtype=torch.float), requires_grad=False)
...
class EIGNN_chain(nn.Module):
    def __init__(self, adj, sp_adj, m, m_y, threshold, max_iter, gamma):
        super(EIGNN_chain, self).__init__()
        # self.B = nn.Parameter(torch.FloatTensor(m_y, m), requires_grad=True)
