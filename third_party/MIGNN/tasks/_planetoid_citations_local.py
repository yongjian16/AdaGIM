...
from datasets._planetoid_citations_local import planetoid_citation_dataloaders
sys.path.append('/scratch/yongjian/projects/implicitLimitation/neurips2026/third_party/MIGNN/agg/')
...
def load(cfg):
    ...
    dataset, _, _, _, _ = planetoid_citation_dataloaders(
        name=args['dataset'],
        adjacency='symm-norm',
        split='geom-gcn'  # match the paper's reported protocol
    )
    # Set Model
...
def run_folds(cfg):
    # The author's load() always uses geom-gcn here (10-fold masks).
    is_public = False
    kfolds = 10
    ...
    for k in range(kfolds):
        ...
        n_te = int(test_mask.sum().item())
        total = n_tr + n_val + n_te
        print(f'Fold {k} Splits: train({100*n_tr/total:.2f})\tval({100*n_val/total:.2f})\ttest({100*n_te/total:.2f})\ttrv({n_tr+n_val})')
...
@hydra.main(version_base=None, config_path="/scratch/yongjian/projects/implicitLimitation/neurips2026/third_party/MIGNN/tasks/", config_name="_planetoid_citations_local")
def run_planetoid(cfg):
    # Initialize settings to wandb server (disabled for our reproduction run)
    cfg.setup['sweep'] = False
    mode = 'disabled'
    wandb.init(
        dir='/tmp/wandb_mignn',
        mode=mode,
